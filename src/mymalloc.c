...
/* Notes
  1. Block->size is the payload size (does not include metadata size)
  2.
    - malloc use next and prev pointer to traverse free block
    - Coalesce use Block->size to traverse all block and merge with left and right
*/
  ...
  if (best){
    if (best->size > size){
          ...
          if (leftover >= (kMinAllocationSize + kMetadataSize)){
              Block * nBlock    = (Block *)((char *)best + kMetadataSize + size);
              nBlock->size      = leftover - kMetadataSize;
...
void coalesce(){
    ...
    while (arena){
        ...
        while (blk->size != 0){
            Block * r_blk = (Block *)((char *)blk + kMetadataSize + blk->size);
            if (r_blk->size == 0) 
                break;
            if (!blk->allocated && !r_blk->allocated ){
                removeNode(blk);
                removeNode(r_blk);

                blk->size += r_blk->size + kMetadataSize;
                blk->next = freeList;
                blk->prev = NULL;
                if (freeList) 
                    freeList->prev = blk;
...
Block *get_next_block(Block *block) {
    ...
    Block * next_block = (Block *) ((char *) block + kMetadataSize + block->size);
